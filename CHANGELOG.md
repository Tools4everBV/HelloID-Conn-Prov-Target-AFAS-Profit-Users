...
- Updated import logic to process and output accounts directly during page retrieval, removing intermediate `$importedAccounts` storage for improved performance and reduced memory usage

### Fixed

- Fixed account reference in `import.ps1` to match the format used in the account lifecycle scripts

## [3.1.0] - 2025-07-16

### Added

- Added `import.ps1` for importing existing AFAS Profit user accounts into HelloID
...
- Encoded `aRef` value in the GET call to handle special characters in account references

## [3.0.1] - 2025-03-06

### Changed

- Updated `fieldMapping.json` with improved field definitions and mandatory descriptions
- Removed password field from field mapping as it is no longer required
- Updated README with remarks about InSite and Profit Windows configuration and improved logo and table of contents

### Fixed

- Fixed typo in `create.ps1`
- Fixed mapping checks to correctly evaluate person properties (issue [#24](https://github.com/Tools4everBV/HelloID-Conn-Prov-Target-AFAS-Profit-Users/issues/24))
- Removed redundant audit log entries in update action when there are no changes (issue [#23](https://github.com/Tools4everBV/HelloID-Conn-Prov-Target-AFAS-Profit-Users/issues/23))
- Fixed `currentAccount` to correctly reflect the value in output data (issue [#21](https://github.com/Tools4everBV/HelloID-Conn-Prov-Target-AFAS-Profit-Users/issues/21))

## [3.0.0] - 2024-06-21

### Added

- Full rewrite of the connector to PowerShell V2 (PSv2) connector standard
- Added `fieldMapping.json` with `UsId` create action and `<CustomerCode>` placeholder
- Added support for setting and unsetting InSite (`InSi`) on enable and disable actions
- Added `configuration.json` for connector configuration

### Fixed

- Fixed exception handling for AFAS license threshold exceeded errors (issue [#16](https://github.com/Tools4everBV/HelloID-Conn-Prov-Target-AFAS-Profit-Users/issues/16))
- Fixed audit log payload exceeding data size limit
- Renamed `updateOnUpdate` to `onlyUpdateOnCorrelate` for clarity
