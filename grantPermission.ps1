...
try {
    ...
    try {
        ...
        if ($errorMessage.AuditErrorMessage -Like "*No AFAS user found*") {
            $auditLogs.Add([PSCustomObject]@{
                    # Action  = "" # Optional
                    Message = "No AFAS user found where [$($correlationProperty)] = [$($aRef.Gebruiker)]. Possibly deleted."
                    IsError = $true
                })
        }
        else {
    ...
    try {
        ...
        if (-not($dryRun -eq $true)) {
            Write-Verbose "Granting permission [$($pRef.Name)] to AFAS user [$($currentAccount.Gebruiker)]"

            $grantPermission = Invoke-RestMethod @splatWebRequest -Verbose:$false

            $auditLogs.Add([PSCustomObject]@{
                    # Action  = "" # Optional
                    Message = "Successfully granted permission [$($pRef.Name)] to AFAS user [$($currentAccount.Gebruiker)]"
                    IsError = $false
                })
        }
        else {
            Write-Warning "DryRun: Would grant permission [$($pRef.Name)] to AFAS user [$($currentAccount.Gebruiker)]"
        }
    }
    catch {
        $ex = $PSItem
        ...
        $auditLogs.Add([PSCustomObject]@{
                # Action  = "" # Optional
                Message = "Error granting permission [$($pRef.Name)] to AFAS user [$($currentAccount.Gebruiker)]. Error Message: $($errorMessage.AuditErrorMessage)"
                IsError = $true
            })
    }
}
